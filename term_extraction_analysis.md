...
## 提取方式概览
- `app_core/term_extraction.py` 中的 `ds_extract_terms` 默认调用 DeepSeek Chat 生成式接口抽取 JSON 术语；未提供 Key 或明确 `prefer_corpus_model=True` 时，会自动退回本地语料向量模型，保持可用性。【F:app_core/term_extraction.py†L157-L261】
- `extract_terms_with_corpus_model` 使用本地语料向量模型（SentenceTransformer）为输入文本和候选术语生成向量，再按照语义相似度排序返回结构化条目；在抽取时会用跨语种向量相似度先对中英文候选做一对一匹配，避免出现“中文对中文、英文对英文”的同语种对照。【F:app_core/term_extraction.py†L24-L154】
- 候选术语通过正则提取：
  - 中文连续 2-8 字符。
...
4. **调用参数顺序出错**：此前 UI 侧将“文本、Key、模型”传给 `ds_extract_terms`，但函数参数顺序为“Key、模型、文本”，导致实际参与抽取的只有模型名（如 `deepseek-chat`），长文本被忽略；现已调整为文本优先的签名，避免再次出现这种错位。【F:app_core/term_extraction.py†L157-L192】【F:app_core/ui_history.py†L116-L129】

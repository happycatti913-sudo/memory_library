...
def render_corpus_manager(st, cur, conn, pid_prefix="corpus"):
    ...
    section_labels = ["新建语料", "浏览/检索", "使用与导出"]
    section_choice = st.sidebar.selectbox("📚 语料库分支", section_labels, key=sk("section"))
    ...
    if section_choice == section_labels[0]:
        ...
        with section_containers[1]:
            ...
            proj_opts = {"(请选择加入参考的项目)": None}
            proj_opts.update({f"[{pid}] {ttl}": pid for pid, ttl in proj_rows})
            proj_label_list = list(proj_opts.keys())
            saved_proj_label = st.session_state.get("corpus_ref_target_label")
            if saved_proj_label not in proj_label_list:
                saved_proj_label = proj_label_list[0]
            target_label = st.selectbox(
                "Few-shot 参考集合添加到哪个项目？",
                proj_label_list,
                index=proj_label_list.index(saved_proj_label),
                key=sk("browse_ref_target"),
            )
            st.session_state["corpus_ref_target_label"] = target_label
            st.session_state["corpus_ref_target_project"] = proj_opts.get(target_label)
            if st.session_state["corpus_ref_target_project"]:
                st.caption(f"当前目标项目: {target_label}")
            else:
                st.caption("未选择项目: 请选择目标项目后再点击“加入参考集合”。")
                    ...
                    with st.expander(f"[{rid}] {ttl} | 项目:{pj} | 方向:{lpv} | {ctime}"):
                        ...
                        with c1:
                            if st.button("加入参考集合", key=sk(f"add_ref_{rid}")):
                                target_pid = st.session_state.get("corpus_ref_target_project")
                                if not target_pid:
